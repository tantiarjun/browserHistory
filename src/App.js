...
class App extends Component {
  ...
  render() {
    ...
    const searchResult = searchHistory.filter(eachItem =>
      eachItem.title.toLowerCase().includes(searchInput.toLowerCase()),
    )

    const IncludeList = searchHistory.map(eachItem =>
      eachItem.title.toLowerCase().includes(searchInput.toLowerCase()),
    )

    const result = IncludeList.every(item => item === false)

    return (
      <div>
        {searchHistory.length === 0 || result === true ? (
